...
exports.get_events = async (req, res, next) => {
  ...
  await fs.readFile("credentials.json", async (err, content) => {
    if (err) return console.log("Error loading client secret file:", err);
    // Authorize a client with credentials, then call the Google Calendar API.
    authorize(JSON.parse(content), await listEvents);
  });
   ...
   */
  function getAccessToken(oAuth2Client, callback) {
    oAuth2Client.getToken(tokn, (err, token) => {
      ...
      (err, res) => {
        ...
        if (events.length) {
          ...
          events.map((event, i) => {
            ...
            console.log(
              "startdate",
              Formatedate.format(startdate, "YYYY-MM-DD")
            );
            let endtdate = new Date(event.end.dateTime || event.end.date);
            eventlist.push({
              id: event.id,
              title: event.summary,
              start: Formatedate.format(startdate, "YYYY-MM-DD"),
              end: Formatedate.format(endtdate, "YYYY-MM-DD"),
